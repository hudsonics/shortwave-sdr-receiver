...
bool Si5351::init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr) {
	...
	si5351_write(SI5351_CRYSTAL_LOAD, (xtal_load_c & SI5351_CRYSTAL_LOAD_MASK) | /*0b00010010*/0x12);
...
void Si5351::set_clock_pwr(enum si5351_clock clk, uint8_t pwr)
{
	...
	if(pwr == 1) {
		reg_val &= /*0b01111111*/0x7F;
	} else {
		reg_val |= /*0b10000000*/0x80;
	}
...
void Si5351::set_clock_disable(enum si5351_clock clk, enum si5351_clock_disable dis_state)
{
	...
	if (clk >= SI5351_CLK0 && clk <= SI5351_CLK3) {
		reg_val &= ~(/*0b11*/0x3 << (clk * 2));
		reg_val |= dis_state << (clk * 2);
	} else if(clk >= SI5351_CLK4 && clk <= SI5351_CLK7) {
		reg_val &= ~(/*0b11*/0x3 << ((clk - 4) * 2));
...
// I2C communication functions (si5351_write, si5351_write_bulk, si5351_read) are implemented in si5351_rx231.cpp - RX231 hardware abstraction layer
